...
/**
 * ShootingStar theme variables.
 *  
*/    
$shootingstar_themename = "ShootingStar Premium";			//Theme Name
$shootingstar_themever = "2.1.6";									//Theme version
$shootingstar_shortname = "shootingstar";							//Shortname 
$shootingstar_manualurl = get_template_directory_uri() . '/docs/documentation.html';	//Manual Url
// Set path to ShootingStar Framework and theme specific functions
$shootingstar_be_path = get_template_directory() . '/functions/be/';									//BackEnd Path
$shootingstar_fe_path = get_template_directory() . '/functions/fe/';									//FrontEnd Path 
$shootingstar_be_pathimages = get_template_directory_uri() . '/functions/be/images';		//BackEnd Path
$shootingstar_fe_pathimages = get_template_directory_uri() . '';	//FrontEnd Path
//Include Framework [BE] 
require_once ($shootingstar_be_path . 'fw-setup.php');		   // Init 
require_once ($shootingstar_be_path . 'fw-options.php');	 	 // Framework Init  
// Include Theme specific functionality [FE] 
require_once ($shootingstar_fe_path . 'headerdata.php');		 // Include css and js
require_once ($shootingstar_fe_path . 'library.php');	       // Include library, functions
require_once ($shootingstar_fe_path . 'widget-twitter.php'); // Twitter Widget
require_once ($shootingstar_fe_path . 'widget-facebook.php');// Facebook Widget
require_once ($shootingstar_fe_path . 'widget-info-box.php');// Info-Box Widget
require_once ($shootingstar_fe_path . 'widget-posts-column.php');// Posts-Column Widget
require_once ($shootingstar_fe_path . 'widget-posts-default.php');// Posts-Default Widget
require_once ($shootingstar_fe_path . 'widget-posts-grid.php');// Posts-Grid Widget
require_once ($shootingstar_fe_path . 'widget-posts-list.php');// Posts-List Widget
require_once ($shootingstar_fe_path . 'widget-posts-slider.php');// Posts-Slider Widget
require_once ($shootingstar_fe_path . 'widget-posts-thumbnail.php');// Posts-Thumbnail Widget
require_once ($shootingstar_fe_path . 'google-map-shortcode.php');// Google Maps Shortcode
require_once ($shootingstar_fe_path . '/wp-display-header/wp-display-header.php');// Different Header Images for individual pages
require_once ($shootingstar_fe_path . '/wp-tab-widget/wp-tab-widget.php');// Tab Widget
...
function shootingstar_setup() {
  ...
  add_image_size( 'slider-thumb', 175, 130, true );
...
function shootingstar_scripts_styles() {
	global $wp_styles, $wp_scripts, $shootingstar_options;
  ...
  if ( $shootingstar_infinite_scroll != 'Disable' ) {
  if ( is_home() || is_archive() || is_search() ) {
  wp_enqueue_script( 'shootingstar-infinitescroll', get_template_directory_uri() . '/js/infinitescroll.js', array( 'jquery' ), '2.0.2', true );
  ...
  if ( $shootingstar_display_carousel != 'Hide' && !is_page_template('template-landing-page.php') ) {  
  wp_enqueue_script( 'shootingstar-caroufredsel', get_template_directory_uri() . '/js/caroufredsel.js', array( 'jquery' ), '6.2.1', true );
  wp_enqueue_script( 'shootingstar-caroufredsel-settings', get_template_directory_uri() . '/js/caroufredsel-settings.js', array(), '1.0', true ); }
  wp_enqueue_script( 'shootingstar-flexslider', get_template_directory_uri() . '/js/flexslider.js', array( 'jquery' ), '2.2.2', true );
  wp_enqueue_script( 'shootingstar-flexslider-settings', get_template_directory_uri() . '/js/flexslider-settings.js', array( 'jquery' ), '2.2.0', true );
  wp_enqueue_script( 'shootingstar-placeholders', get_template_directory_uri() . '/js/placeholders.js', array( 'jquery' ), '2.0.8', true );
  if ( $shootingstar_display_scroll_top != 'Hide' ) {
  wp_enqueue_script( 'shootingstar-scroll-to-top', get_template_directory_uri() . '/js/scroll-to-top.js', array( 'jquery' ), '1.0', true ); }
  if ( $shootingstar_fixed_menu != 'Disable' && !is_page_template('template-landing-page.php') ) {
  ...
  wp_enqueue_script( 'shootingstar-html5-ie', get_template_directory_uri() . '/js/html5.js', array(), '3.6', false );
  ...
  wp_enqueue_style( 'shootingstar-tab-widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/css/wp-tab-widget.css' );
 ...
 *  
*/
function shootingstar_catch_that_image() {
  global $post, $posts;
  $first_img = '';
  ob_start();
  ob_end_clean();
 ...
 *  
*/
function shootingstar_catch_that_audio() {
global $post, $posts;
 ...
 *
*/
function shootingstar_add_meta_box_slideshow() {
	$screens = array( 'post', 'page' );
	foreach ( $screens as $screen ) {
		add_meta_box(
			'shootingstar_sectionid_slideshow',
			__( 'Header Slideshow', 'shootingstar' ),
			'shootingstar_meta_box_callback_slideshow',
			$screen
		);
	}
}
add_action( 'add_meta_boxes', 'shootingstar_add_meta_box_slideshow' );

function shootingstar_meta_box_callback_slideshow( $post ) {
	wp_nonce_field( 'shootingstar_meta_box_slideshow', 'shootingstar_meta_box_nonce_slideshow' );
	$value = get_post_meta( $post->ID, '_my_meta_value_key_slideshow', true );
	echo '<label for="shootingstar_new_field_slideshow">';
	_e( 'Insert here a Slideshow ID to display a slideshow in header.', 'shootingstar' );
	echo '</label> ';
	echo '<input type="text" id="shootingstar_new_field_slideshow" name="shootingstar_new_field_slideshow" value="' . esc_attr( $value ) . '" size="100" />';
}

function shootingstar_save_meta_box_data_slideshow( $post_id ) {
	if ( ! isset( $_POST['shootingstar_meta_box_nonce_slideshow'] ) ) {
		return;
	}
	if ( ! wp_verify_nonce( $_POST['shootingstar_meta_box_nonce_slideshow'], 'shootingstar_meta_box_slideshow' ) ) {
		return;
	}
	...
	if ( ! isset( $_POST['shootingstar_new_field_slideshow'] ) ) {
		return;
	}
	$my_data = sanitize_text_field( $_POST['shootingstar_new_field_slideshow'] );
	update_post_meta( $post_id, '_my_meta_value_key_slideshow', $my_data );
}
add_action( 'save_post', 'shootingstar_save_meta_box_data_slideshow' );

/**
 * Adds a custom meta box for setting a Video Thumbnail.
 *
*/
function shootingstar_add_meta_box() {
	$screens = array( 'post' );
	foreach ( $screens as $screen ) {
		add_meta_box(
			'shootingstar_sectionid',
			__( 'Video Thumbnail', 'shootingstar' ),
			'shootingstar_meta_box_callback',
			$screen
		);
	}
}
add_action( 'add_meta_boxes', 'shootingstar_add_meta_box' );

function shootingstar_meta_box_callback( $post ) {
	wp_nonce_field( 'shootingstar_meta_box', 'shootingstar_meta_box_nonce' );
	$value = get_post_meta( $post->ID, '_my_meta_value_key', true );
	echo '<label for="shootingstar_new_field">';
	_e( 'Insert here the full URL address of the Video Thumbnail.', 'shootingstar' );
	echo '</label> ';
	echo '<input type="text" id="shootingstar_new_field" name="shootingstar_new_field" value="' . esc_attr( $value ) . '" size="25" />';
}

function shootingstar_save_meta_box_data( $post_id ) {
	if ( ! isset( $_POST['shootingstar_meta_box_nonce'] ) ) {
		return;
	}
	if ( ! wp_verify_nonce( $_POST['shootingstar_meta_box_nonce'], 'shootingstar_meta_box' ) ) {
		return;
	}
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	...
	if ( ! isset( $_POST['shootingstar_new_field'] ) ) {
		return;
	}
	$my_data = sanitize_text_field( $_POST['shootingstar_new_field'] );
	update_post_meta( $post_id, '_my_meta_value_key', $my_data );
}
add_action( 'save_post', 'shootingstar_save_meta_box_data' );
 ...
 *
 */
function shootingstar_register_my_menus() {
  register_nav_menus(
    array(
      'main-navigation' => __( 'Main Header Menu', 'shootingstar' ),
      'top-navigation' => __( 'Social Networks Links', 'shootingstar' ),
      'carousel-navigation' => __( 'Carousel Menu', 'shootingstar' )
    )
  );
}
add_action( 'after_setup_theme', 'shootingstar_register_my_menus' );
...
function shootingstar_widgets_init() {
  ...
  register_sidebar( array(
		'name' => __( 'Homepage slideshow', 'shootingstar' ),
		'id' => 'sidebar-6',
		'description' => __( 'The area for Cyclone Slider Widget which displays a slideshow on your homepage.', 'shootingstar' ),
...
if ($shootingstar_excerpt_length != '') {
return $shootingstar_excerpt_length;
} else { return 40; }
}
 ...
 *
*/
function shootingstar_status_content() {
 ...
 *  
*/
function shootingstar_panels_row_styles($styles) {
    $styles['shootingstar'] = __('ShootingStar', 'shootingstar');
    return $styles;
}
add_filter('siteorigin_panels_row_styles', 'shootingstar_panels_row_styles');

/**
 * Custom Shortcodes.
 *
*/
function shootingstar_button_shortcode($atts, $content = null) {
   extract(shortcode_atts(array('link' => '#'), $atts));
   return '<a class="custom-button" href="'.$link.'">' . do_shortcode($content) . '</a>';
}
add_shortcode( 'button', 'shootingstar_button_shortcode' );
add_filter('widget_text', 'do_shortcode');

function shootingstar_highlight_shortcode($atts, $content = null) {
   return '<span class="highlight-text">' . do_shortcode($content) . '</span>';
}
add_shortcode( 'highlight', 'shootingstar_highlight_shortcode' );

function shootingstar_table_shortcode($atts) {
...
add_shortcode( 'table', 'shootingstar_table_shortcode' );

function shootingstar_image_shortcode($atts, $content = null){
return '<img src="'.$atts['src'].'" alt="" />';
}
add_shortcode('image','shootingstar_image_shortcode');

/**
 * Shortcode for displaying lists of posts.
 *  
*/
add_shortcode( 'display-posts', 'shootingstar_display_posts_shortcode' );
function shootingstar_display_posts_shortcode( $atts ) {
...
$example_update_checker = new ThemeUpdateChecker(
    'shootingstar-premium',
    'http://www.tomastoman.cz/updates/shootingstar.json'
); ?>