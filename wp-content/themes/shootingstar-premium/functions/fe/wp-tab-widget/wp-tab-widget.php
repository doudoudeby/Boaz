<?php
/*
Plugin Name: ShootingStar Tab Widget
...
class shootingstar_widget extends WP_Widget {
	function __construct() {
        
		// ajax functions
		add_action('wp_ajax_shootingstar_widget_content', array(&$this, 'ajax_shootingstar_widget_content'));
		add_action('wp_ajax_nopriv_shootingstar_widget_content', array(&$this, 'ajax_shootingstar_widget_content'));
		...
		$widget_ops = array('classname' => 'widget_wpt', 'description' => __('Display popular posts, recent posts, comments and tags in tabbed format.', 'shootingstar'));
		$control_ops = array('width' => 300, 'height' => 350);
		parent::__construct('shootingstar_widget', __('ShootingStar Tab Widget', 'shootingstar'), $widget_ops, $control_ops);
    }	

    function wpt_admin_scripts($hook) {
        wp_enqueue_script( 'shootingstar_widget_admin', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wpt-admin.js', array( 'jquery' ), '1.0', true );
    }
    function wpt_register_scripts() { 
		// JS    
		wp_enqueue_script('shootingstar_widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wp-tab-widget.js', array('jquery'), '1.0', true);     
		wp_localize_script( 'shootingstar_widget', 'wpt',         
	...
	function form( $instance ) {
        ...
        <h4><?php _e('Select Tabs', 'shootingstar'); ?></h4>
		...
		<div class="wpt_select_tabs">
			<label class="alignleft" style="display: block; width: 50%; margin-bottom: 5px" for="<?php echo $this->get_field_id("tabs"); ?>_popular">
				...
				<?php _e( 'Popular Tab', 'shootingstar'); ?>
			</label>
			<label class="alignleft" style="display: block; width: 50%; margin-bottom: 5px;" for="<?php echo $this->get_field_id("tabs"); ?>_recent">
				...
				<?php _e( 'Recent Tab', 'shootingstar'); ?>
			</label>
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_comments">
				...
				<?php _e( 'Comments Tab', 'shootingstar'); ?>
			</label>
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_tags">
				...
				<?php _e( 'Tags Tab', 'shootingstar'); ?>
			</label>
		</div>
        <div class="clear"></div>
        
        <h4 class="wpt_tab_order_header"><a href="#"><?php _e('Tab Order', 'shootingstar'); ?></a></h4>
        ...
        <div class="wpt_tab_order" style="display: none;">
            ...
            <label class="alignleft" for="<?php echo $this->get_field_id('tab_order'); ?>_tags" style="width: 50%;">
			    ...
			    <?php _e('Tags', 'shootingstar'); ?>
            </label>
        </div>
		<div class="clear"></div>
        
        <h4 class="wpt_advanced_options_header"><a href="#"><?php _e('Advanced Options', 'shootingstar'); ?></a></h4>
        ...
        <p>
			<label for="<?php echo $this->get_field_id("allow_pagination"); ?>">				
				...
				<?php _e( 'Allow pagination', 'shootingstar'); ?>
			</label>
		</p>
        ...
        <p>
			<label for="<?php echo $this->get_field_id('post_num'); ?>"><?php _e('Number of posts to show:', 'shootingstar'); ?>
				<br />
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_thumb"); ?>">
				...
				<?php _e( 'Show post thumbnails', 'shootingstar'); ?>
			</label>
		</p>   	
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_date"); ?>">	
				...
				<?php _e( 'Show post date', 'shootingstar'); ?>	
			</label>	
		</p>
		...
		<p>		
			<label for="<?php echo $this->get_field_id("show_comment_num"); ?>">		
				...
				<?php _e( 'Show number of comments', 'shootingstar'); ?>		
			</label>	
		</p>    
        ...
        <p>
			<label for="<?php echo $this->get_field_id('comment_num'); ?>">
				<?php _e('Number of comments on Comments Tab:', 'shootingstar'); ?>
				<br />
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_avatar"); ?>">			
				...
				<?php _e( 'Show avatars on Comments Tab', 'shootingstar'); ?>	
			</label>	
		</p>
	...
	function widget( $args, $instance ) {	
		extract($args);     
		extract($instance);    
		wp_enqueue_script('shootingstar_widget');  
        ...
        $available_tabs = array('popular' => __('Popular', 'shootingstar'), 
            'recent' => __('Recent', 'shootingstar'), 
            'comments' => __('Comments', 'shootingstar'), 
            'tags' => __('Tags', 'shootingstar'));
            
        array_multisort($tab_order, $available_tabs);
        
		?>	
		<?php echo $before_widget; ?>	
		<div class="shootingstar_widget_content" id="<?php echo $widget_id; ?>_content">		
	...
	function ajax_shootingstar_widget_content() {     
		$tab = $_POST['tab'];       
		$args = $_POST['args'];  
		...
		switch ($tab) {        
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>	
								<div class="wpt-postmeta">						
									...
									<?php if ( $show_comment_num == 1 ) : ?>			
										<?php echo comments_number(__('No Comment','shootingstar'), __('One Comment','shootingstar'), '<span class="comments-number">%</span> '.__('Comments','shootingstar'));?>				
									<?php endif; ?>						
								</div> <!--end .entry-meta--> 				
							<?php endif; ?>
						...
						<li>
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>			
								<div class="wpt-postmeta">										
									...
									<?php endif; ?>								
									<?php if ( $show_comment_num == 1 ) : ?>	
										<?php echo comments_number(__('No Comment','shootingstar'), __('One Comment','shootingstar'), '<span class="comm">%</span> '.__('Comments','shootingstar'));?>									
									<?php endif; ?>		
								</div> <!--end .entry-meta--> 		
							<?php endif; ?>
			...
			case "comments":         
				...
				<ul>            
					...
					<?php endforeach; else : ?>           
						<li>                   
							<div class="no-comments"><?php _e('No comments yet.', 'shootingstar'); ?></div>        
						</li>                             
			...
			case "tags":        
				...
				<ul>         
					...
					if($tags) {               
						...
						_e('No tags created.', 'shootingstar');           
					}            
					?>           
				</ul>            
				<?php            
    ...
    function tab_pagination($page, $last_page) {  
		...
		<div class="wpt-pagination">     
			<?php if ($page > 1) : ?>               
				<a href="#" class="previous"><span><?php _e('&larr; Previous', 'shootingstar'); ?></span></a>      
			<?php endif; ?>        
			<?php if ($page != $last_page) : ?>     
				<a href="#" class="next"><span><?php _e('Next &rarr;', 'shootingstar'); ?></span></a>      
			<?php endif; ?>          
		</div>                   
		<div class="clear"></div>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "shootingstar_widget" );' ) );
