<?php
/**
 * Plugin Name: ShootingStar Twitter Widget
...
?>
<?php 
add_action('widgets_init', create_function('', 'return register_widget("shootingstartwitter");'));
class shootingstartwitter extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'shootingstartwitter', 'description' => __('Twitter widget by ShootingStar', 'shootingstar') );
		$control_ops = array('width' => 200, 'height' => 400);
		parent::__construct('shootingstarttwi', __('ShootingStar Twitter Widget', 'shootingstar'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		if ( $instance ) {
			...
			$title = __( 'Join us on Twitter', 'shootingstar' );
		} 
		...
		<p>
			<label for="<?php echo $this->get_field_id('username'); ?>">
				<?php _e('Twitter Username:', 'shootingstar'); ?>
			</label>
			<input class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" type="text" value="<?php echo $username; ?>" />
		<p style="font-size: 10px; color: #999; margin: 0; padding: 0px;">
			<?php _e('Insert your Twitter username. (ex: shootingstar).', 'shootingstar'); ?>
		</p>
		</p>
		<?php 
	} 
	...
	function widget($args, $instance) {
		...
		function shootingstarmakeURL($url) {
		...
		foreach ($feed->get_items(0,$tweets) as $item) { ?>
			<p class="tweet-text"><?php echo shootingstaruserurl(shootingstartagurl(shootingstarmakeURL($item->get_description()))); ?> 
