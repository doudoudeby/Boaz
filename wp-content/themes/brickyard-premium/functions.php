...
$brickyard_themever = "2.0.3";									//Theme version
 ...
 *
*/
function brickyard_add_meta_box_slideshow() {
	$screens = array( 'post', 'page' );
	foreach ( $screens as $screen ) {
		add_meta_box(
			'brickyard_sectionid_slideshow',
			__( 'Header Slideshow', 'brickyard' ),
			'brickyard_meta_box_callback_slideshow',
			$screen
		);
	}
}
add_action( 'add_meta_boxes', 'brickyard_add_meta_box_slideshow' );

function brickyard_meta_box_callback_slideshow( $post ) {
	wp_nonce_field( 'brickyard_meta_box_slideshow', 'brickyard_meta_box_nonce_slideshow' );
	$value = get_post_meta( $post->ID, '_my_meta_value_key_slideshow', true );
	echo '<label for="brickyard_new_field_slideshow">';
	_e( 'Insert here a Slideshow ID to display a slideshow in header.', 'brickyard' );
	echo '</label> ';
	echo '<input type="text" id="brickyard_new_field_slideshow" name="brickyard_new_field_slideshow" value="' . esc_attr( $value ) . '" size="100" />';
}

function brickyard_save_meta_box_data_slideshow( $post_id ) {
	if ( ! isset( $_POST['brickyard_meta_box_nonce_slideshow'] ) ) {
		return;
	}
	if ( ! wp_verify_nonce( $_POST['brickyard_meta_box_nonce_slideshow'], 'brickyard_meta_box_slideshow' ) ) {
		return;
	}
	...
	if ( ! isset( $_POST['brickyard_new_field_slideshow'] ) ) {
		return;
	}
	$my_data = sanitize_text_field( $_POST['brickyard_new_field_slideshow'] );
	update_post_meta( $post_id, '_my_meta_value_key_slideshow', $my_data );
}
add_action( 'save_post', 'brickyard_save_meta_box_data_slideshow' );
...
if ($brickyard_excerpt_length != '') {
return $brickyard_excerpt_length;
} else { return 40; }
}
...
add_action( 'brickyard_register', 'brickyard_my_theme_register_required_plugins' );
