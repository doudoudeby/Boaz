<?php
/**
 * Plugin Name: HappenStance Info-Box Widget
 * Description: Displays a box with your custom text and link.
 * Author: Tomas Toman	
 * Version: 1.0
*/

add_action('widgets_init', create_function('', 'return register_widget("happenstance_info_box");'));
class happenstance_info_box extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'homepage-info-box', 'description' => __('Displays a box with your custom text and icon.', 'happenstance') );
		$control_ops = array('width' => 200, 'height' => 400);
		parent::__construct('infobox', __('HappenStance Info-Box', 'happenstance'), $widget_ops, $control_ops);
	}
    ...
    if ( $instance ) {
	  ...
	  if ( $instance ) {
			...
			$content = __( '', 'happenstance' );
		} ?>
<p>
	<label for="<?php echo $this->get_field_id('icon'); ?>">
		<?php _e('Icon:', 'happenstance'); ?>
	</label>
  ...
  <p style="font-size: 10px; color: #999; margin: 0; padding: 0px;">
	<?php _e('Insert here the name of an icon (example: <strong>icon_calendar</strong>).', 'happenstance'); ?>
</p>
</p>
<!-- Title -->
<p>
