<?php
/**
 * Plugin Name: PhotoStory Twitter Widget
...
?>
<?php 
add_action('widgets_init', create_function('', 'return register_widget("photostorytwitter");'));
class photostorytwitter extends WP_Widget {
	function photostorytwitter() {
		$widget_ops = array('classname' => 'photostorytwitter', 'description' => __('Twitter widget by PhotoStory', 'photostory') );
		$control_ops = array('width' => 200, 'height' => 400);
		$this->WP_Widget('photostoryttwi', __('PhotoStory Twitter Widget', 'photostory'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		if ( $instance ) {
			...
			$title = __( 'Join us on Twitter', 'photostory' );
		} 
		...
		<p>
			<label for="<?php echo $this->get_field_id('username'); ?>">
				<?php _e('Twitter Username:', 'photostory'); ?>
			</label>
			<input class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" type="text" value="<?php echo $username; ?>" />
		<p style="font-size: 10px; color: #999; margin: 0; padding: 0px;">
			<?php _e('Insert your Twitter username. (ex: photostory).', 'photostory'); ?>
		</p>
		</p>
		<?php 
	} 
	...
	function widget($args, $instance) {
		...
		function photostorymakeURL($url) {
		...
		foreach ($feed->get_items(0,$tweets) as $item) { ?>
			<p class="tweet-text"><?php echo photostoryuserurl(photostorytagurl(photostorymakeURL($item->get_description()))); ?> 
