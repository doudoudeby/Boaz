<?php
/*
Plugin Name: PhotoStory Tab Widget
...
class photostory_widget extends WP_Widget {
	function __construct() {
        
		// ajax functions
		add_action('wp_ajax_photostory_widget_content', array(&$this, 'ajax_photostory_widget_content'));
		add_action('wp_ajax_nopriv_photostory_widget_content', array(&$this, 'ajax_photostory_widget_content'));
		...
		$widget_ops = array('classname' => 'widget_wpt', 'description' => __('Display popular posts, recent posts, comments and tags in tabbed format.', 'photostory'));
		$control_ops = array('width' => 300, 'height' => 350);
		$this->WP_Widget('photostory_widget', __('PhotoStory Tab Widget', 'photostory'), $widget_ops, $control_ops);
    }	

    function wpt_admin_scripts($hook) {
        wp_enqueue_script( 'photostory_widget_admin', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wpt-admin.js', array( 'jquery' ), '1.0', true );
    }
    function wpt_register_scripts() { 
		// JS    
		wp_enqueue_script('photostory_widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wp-tab-widget.js', array('jquery'), '1.0', true);     
		wp_localize_script( 'photostory_widget', 'wpt',         
	...
	function form( $instance ) {
        ...
        <h4><?php _e('Select Tabs', 'photostory'); ?></h4>
		...
		<div class="wpt_select_tabs">
			...
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_comments">
				...
				<?php _e( 'Comments Tab', 'photostory'); ?>
			</label>
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_tags">
				...
				<?php _e( 'Tags Tab', 'photostory'); ?>
			</label>
		</div>
        <div class="clear"></div>
        
        <h4 class="wpt_tab_order_header"><a href="#"><?php _e('Tab Order', 'photostory'); ?></a></h4>
        ...
        <div class="wpt_tab_order" style="display: none;">
            ...
            <label class="alignleft" for="<?php echo $this->get_field_id('tab_order'); ?>_tags" style="width: 50%;">
			    ...
			    <?php _e('Tags', 'photostory'); ?>
            </label>
        </div>
		<div class="clear"></div>
        
        <h4 class="wpt_advanced_options_header"><a href="#"><?php _e('Advanced Options', 'photostory'); ?></a></h4>
        ...
        <p>
			<label for="<?php echo $this->get_field_id("allow_pagination"); ?>">				
				...
				<?php _e( 'Allow pagination', 'photostory'); ?>
			</label>
		</p>
        ...
        <p>
			<label for="<?php echo $this->get_field_id('post_num'); ?>"><?php _e('Number of posts to show:', 'photostory'); ?>
				<br />
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_thumb"); ?>">
				...
				<?php _e( 'Show post thumbnails', 'photostory'); ?>
			</label>
		</p>   	
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_date"); ?>">	
				...
				<?php _e( 'Show post date', 'photostory'); ?>	
			</label>	
		</p>
		...
		<p>		
			<label for="<?php echo $this->get_field_id("show_comment_num"); ?>">		
				...
				<?php _e( 'Show number of comments', 'photostory'); ?>		
			</label>	
		</p>    
        ...
        <p>
			<label for="<?php echo $this->get_field_id('comment_num'); ?>">
				<?php _e('Number of comments on Comments Tab:', 'photostory'); ?>
				<br />
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_avatar"); ?>">			
				...
				<?php _e( 'Show avatars on Comments Tab', 'photostory'); ?>	
			</label>	
		</p>
	...
	function widget( $args, $instance ) {	
		extract($args);     
		extract($instance);    
		wp_enqueue_script('photostory_widget');  
        ...
        $available_tabs = array('popular' => __('Popular', 'photostory'), 
            'recent' => __('Recent', 'photostory'), 
            'comments' => __('Comments', 'photostory'), 
            'tags' => __('Tags', 'photostory'));
            
        array_multisort($tab_order, $available_tabs);
        
		?>	
		<?php echo $before_widget; ?>	
		<div class="photostory_widget_content" id="<?php echo $widget_id; ?>_content">		
	...
	function ajax_photostory_widget_content() {     
		$tab = $_POST['tab'];       
		$args = $_POST['args'];  
		...
		switch ($tab) {        
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>	
								<div class="wpt-postmeta">						
									...
									<?php if ( $show_comment_num == 1 ) : ?>			
										<?php echo comments_number(__('No Comment','photostory'), __('One Comment','photostory'), '<span class="comments-number">%</span> '.__('Comments','photostory'));?>				
									<?php endif; ?>						
								</div> <!--end .entry-meta--> 				
							<?php endif; ?>
						...
						<li>
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>			
								<div class="wpt-postmeta">										
									...
									<?php endif; ?>								
									<?php if ( $show_comment_num == 1 ) : ?>	
										<?php echo comments_number(__('No Comment','photostory'), __('One Comment','photostory'), '<span class="comm">%</span> '.__('Comments','photostory'));?>									
									<?php endif; ?>		
								</div> <!--end .entry-meta--> 		
							<?php endif; ?>
			...
			case "comments":         
				...
				<ul>            
					...
					<?php endforeach; else : ?>           
						<li>                   
							<div class="no-comments"><?php _e('No comments yet.', 'photostory'); ?></div>        
						</li>                             
    ...
    function tab_pagination($page, $last_page) {  
		...
		<div class="wpt-pagination">     
			<?php if ($page > 1) : ?>               
				<a href="#" class="previous"><span><?php _e('&larr; Previous', 'photostory'); ?></span></a>      
			<?php endif; ?>        
			<?php if ($page != $last_page) : ?>     
				<a href="#" class="next"><span><?php _e('Next &rarr;', 'photostory'); ?></span></a>      
			<?php endif; ?>          
		</div>                   
		<div class="clear"></div>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "photostory_widget" );' ) );

// unregister MTS Tabs Widget and Tabs Widget v2
add_action('widgets_init', 'photostory_unregister_mts_tabs_widget', 100);
function photostory_unregister_mts_tabs_widget() {
