...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'photostory-install-plugins';
         ...
         * does the photostory_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
        ...
        public function __construct() {
            ...
            do_action_ref_array( 'photostory_init', array( $this ) );
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'photostory_dismissed_notice', true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            if ( isset( $_POST['action'] ) && 'photostory-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="photostory wrap">
                ...
                <form id="photostory-plugins" action="" method="post">
                    <input type="hidden" name="photostory-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['photostory-install'] ) && 'install-plugin' == $_GET['photostory-install'] ) ) {
                check_admin_referer( 'photostory-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'photostory-install' => 'install-plugin',
                        ),
                ...
                $fields = array( 'photostory-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'photostory' ) . '">' . __( 'Return to the Dashboard', 'photostory' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['photostory-activate'] ) && 'activate-plugin' == $_GET['photostory-activate'] ) ) {
                check_admin_referer( 'photostory-activate', 'photostory-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'photostory' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'photostory' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'photostory' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'photostory_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'photostory-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'photostory', 'photostory', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'photostory', 'photostory', $rendered, $nag_class );
                }
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['photostory-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'photostory_dismissed_notice', 1 );
            }
         ...
         */
        protected function is_photostory_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'photostory_dismissed_notice' );
    ...
    $photostory = TGM_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'photostory_tgmpa' ) ) {
    /**
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'photostory' ) : __( 'Recommended', 'photostory' );
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'photostory' ) . ' %2$s">' . __( 'Install', 'photostory' ) . '</a>',
                        esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'photostory' ) . ' %2$s">' . __( 'Activate', 'photostory' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
                                    ...
                                    'plugin_name'          => urlencode( $item['sanitized_plugin'] ),
                                    'plugin_source'        => urlencode( $item['url'] ),
                                    'photostory-activate'       => 'activate-plugin',
                                    'photostory-activate-nonce' => urlencode( wp_create_nonce( 'photostory-activate' ) ),
                                ),
                                network_admin_url( 'themes.php' )
                            )
                        ),
        ...
        public function column_cb( $item ) {
            ...
            if ( __( 'Private Repository', 'photostory' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            } elseif ( __( 'Pre-Packaged', 'photostory' ) === $item['source'] ) {
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'photostory' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'photostory-bulk-install'  => __( 'Install', 'photostory' ),
                'photostory-bulk-activate' => __( 'Activate', 'photostory' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'photostory-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => urlencode( TGM_Plugin_Activation::$instance->menu ),
                            'photostory-action'  => 'install-selected',
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'photostory-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'photostory-empty';
                    $i++;
                }
                ...
                echo '<div class="photostory wrap">';
            ...
            if ( 'photostory-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
...
add_action( 'admin_init', 'photostory_load_bulk_installer' );
if ( ! function_exists( 'photostory_load_bulk_installer' ) ) {
    function photostory_load_bulk_installer() {
    /**
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function bulk_footer() {
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'photostory' ) . '">' . __( 'Return to the Dashboard', 'photostory' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
