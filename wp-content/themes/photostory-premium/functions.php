 ...
 *  
*/    
$photostory_themename = "PhotoStory Premium";			//Theme Name
$photostory_themever = "1.1.7";									//Theme version
...
require_once ($photostory_be_path . 'fw-setup.php');		   // Init 
...
require_once ($photostory_fe_path . 'widget-social.php');	 // Social Widget
require_once ($photostory_fe_path . 'widget-twitter.php'); // Twitter Widget
require_once ($photostory_fe_path . 'widget-facebook.php');// Facebook Widget
require_once ($photostory_fe_path . 'widget-info-box.php');// Info-Box Widget
require_once ($photostory_fe_path . 'widget-posts-column.php');// Posts-Column Widget
require_once ($photostory_fe_path . 'widget-posts-default.php');// Posts-Default Widget
require_once ($photostory_fe_path . 'widget-posts-grid.php');// Posts-Grid Widget
require_once ($photostory_fe_path . 'widget-posts-list.php');// Posts-List Widget
require_once ($photostory_fe_path . 'widget-posts-slider.php');// Posts-Slider Widget
require_once ($photostory_fe_path . 'widget-posts-thumbnail.php');// Posts-Thumbnail Widget
require_once ($photostory_fe_path . 'google-map-shortcode.php');// Google Maps Shortcode
require_once ($photostory_fe_path . '/wp-tab-widget/wp-tab-widget.php');// Tab Widget
...
function photostory_setup() {
  ...
  add_image_size( 'grid-thumb', 250, 188, true );
  add_image_size( 'square-thumb', 60, 60, true );
  add_image_size( 'slider-thumb', 125, 94, true );
  add_image_size( 'thumbnail-thumb', 262, 197, true );
}
...
function photostory_scripts_styles() {
    ...
    wp_enqueue_script( 'photostory-flexslider', get_template_directory_uri() . '/js/flexslider.js', array( 'jquery' ), '2.2.0', true );
    wp_enqueue_script( 'photostory-flexslider-settings', get_template_directory_uri() . '/js/flexslider-settings.js', array( 'jquery' ), '2.2.0', true );
    wp_enqueue_script( 'photostory-placeholders', get_template_directory_uri() . '/js/placeholders.js', array(), '3.0.2', true );
    ...
    wp_enqueue_style( 'photostory-tab-widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/css/wp-tab-widget.css' );
}
...
function photostory_widgets_init() {
  ...
  register_sidebar( array(
		'name' => __( 'Homepage slideshow', 'photostory' ),
		'id' => 'sidebar-7',
		'description' => __( 'The area for Cyclone Slider Widget which displays a slideshow on your homepage.', 'photostory' ),
 ...
 *
*/
function photostory_short_title() {
$title = get_the_title();
$limit = "30";
$pad = "...";
...
add_action( 'photostory_register', 'photostory_my_theme_register_required_plugins' );
 ...
 *  
*/
function photostory_panels_row_styles($styles) {
    $styles['photostory'] = __('PhotoStory', 'photostory');
    return $styles;
}
add_filter('siteorigin_panels_row_styles', 'photostory_panels_row_styles');

/**
 * Custom Shortcodes.
 *
*/
function photostory_button_shortcode($atts, $content = null) {
   extract(shortcode_atts(array('link' => '#'), $atts));
   return '<a class="custom-button" href="'.$link.'">' . do_shortcode($content) . '</a>';
}
add_shortcode( 'button', 'photostory_button_shortcode' );
add_filter('widget_text', 'do_shortcode');

function photostory_highlight_shortcode($atts, $content = null) {
   return '<span class="highlight-text">' . do_shortcode($content) . '</span>';
}
add_shortcode( 'highlight', 'photostory_highlight_shortcode' );

function photostory_table_shortcode($atts) {
...
add_shortcode( 'table', 'photostory_table_shortcode' );

function photostory_image_shortcode($atts, $content = null){
return '<img src="'.$atts['src'].'" alt="" />';
}
add_shortcode('image','photostory_image_shortcode');

/**
 * Shortcode for displaying lists of posts.
 *  
*/
add_shortcode( 'display-posts', 'photostory_display_posts_shortcode' );
function photostory_display_posts_shortcode( $atts ) {
...
$example_update_checker = new ThemeUpdateChecker(
    'photostory-premium',
    'http://www.tomastoman.cz/updates/photostory.json'
); ?>